...
export function HandCard({ cardId, onSelect, disabled, selected, ariaLabel }: HandCardProps) {
  ...
  return (
      ...
      exit={{ opacity: 0, y: -36, rotate: 6 }}
			...
			variants={{ hover: { scale: 1.12, rotate: 0.6 } }}
      type="button"
			className={cn(
        ...
        selected && "scale-[1.05] -translate-y-3 border-[var(--color-accent)] shadow-[0_0_20px_rgba(215,178,110,0.5)] ring-1 ring-[var(--color-accent)]",
    ...
    >
      ...
      <div className="absolute inset-0 overflow-hidden rounded-[22px]">
		...
		<div className="absolute left-3 top-3 z-20 text-[var(--color-accent-light)]">
			<span className="font-heading text-3xl drop-shadow" aria-hidden={true}>
				{definition.rank}
			</span>
		...
		<motion.div
			...
			initial="rest"
			variants={{ rest: { opacity: 1, scale: 1 }, hover: { opacity: 1, scale: 1.25 } }}
			transition={{ duration: 0.16, ease: "easeOut" }}
		>
			<CardSymbol icon={definition.icon} size={38} className="text-[var(--color-accent-light)] drop-shadow" />
		</motion.div>
		<motion.p
